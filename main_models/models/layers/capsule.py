...
from models.layers.neural import BatchMLP
from models.inferencer_1 import QKVAttention
...
class CapsuleLayer(snt.AbstractModule):
  ...
  def _build(self, features, parent_transform=None, parent_presence=None):
    ...
    output_shapes = (
        [self._n_votes, self._n_transform_params],  # CPR_dynamic [96, 6]
        [self._n_votes, self._n_transform_params],  # CCR [1, 6]
        [1],  # per-capsule presence [1]
        [self._n_votes],  # per-vote scale [96]
    )
    ...
    # ccr: OV matrix, pres_logit_per_caps: a_k, pres_logit_per_vote: a_kn, scale_per_vote: c_k
    ccr, pres_logit_per_caps, scale_per_vote = res
    # CCR [1, 6] per-capsule presence [1] per-vote-presence [96] per-vote scale [96]
    if self._caps_dropout_rate != 0.0:
    ...
    cpr = self._make_transform(cpr_dynamic)

    #ccr_per_vote = snt.TileByDim([2], [self._n_votes])(ccr)
    ccr_per_vote = tf.nn.softplus(ccr)
    votes = tf.matmul(ccr_per_vote, cpr) # v_kn = OV_k * OP_kn
    #votes = snt.BatchApply(snt.Linear(640))(votes)
    ...
    pres_per_vote = tf.nn.sigmoid(pres_per_caps) # a_k * a_kn

    scale_per_vote = tf.nn.softplus(scale_per_vote + .5) + 1e-2


    return AttrDict(
        vote = votes, # v_kn = OV_k * OP_kn  mean value for Gaussian
        scale = scale_per_vote, # c_k Variance for Gaussian
...
class OrderInvariantCapsuleLikelihood(snt.AbstractModule):
  ...
  def _build(self, x, pres_logit_per_vote=None, pres_logit_per_caps=None, presence=None, state=None):
...
class CapsuleLikelihood(OrderInvariantCapsuleLikelihood):
  ...
  def _build(self, x,
             pres_logit_per_vote=None,
             pres_logit_per_caps=None,
             presence=None, state=None):
    ...
    vote_component_pdf = self._get_pdf(self._votes, # (N,128,96,6)
                                       tf.expand_dims(self._scales, -1)) #(N,128,96)

    # expand along caps dimensions -> [B, 1, n_input_points, n_input_dims]
    x = snt.BatchApply(snt.Linear(128))(x)
    x = snt.BatchApply(snt.Linear(6))(x)
    expanded_x = tf.expand_dims(x, 1)
    #vote_prob_per_dim = vote_component_pdf.prob(expanded_x)
    vote_prob_per_dim = vote_component_pdf.cdf(expanded_x)

    mu = vote_component_pdf.mean()
    var = vote_component_pdf.variance()
    std = vote_component_pdf.stddev()
    mix_coeff = tf.nn.softmax(tf.squeeze(presence, -1), axis = -1)

    eps = tf.random_normal(shape = tf.shape(std))
    z = mu + eps * std

    z = tf.reduce_sum(z, 2)
    z = snt.BatchApply(snt.Linear(128))(z)
    #mix_coeff = tf.nn.softmax(mix_coeff, axis = -1)
    mix_coeff = tf.expand_dims(mix_coeff, -1)
    mix_coeff = tf.tile(mix_coeff, multiples = (1,1,128))

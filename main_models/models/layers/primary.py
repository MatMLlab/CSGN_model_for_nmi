...
from models.layers import math_ops
from models.layers import prob
from models.layers.tensor_ops import make_brodcastable
from models.inferencer_1 import SetTransformer
...
class CapsuleMaterialEncoder(snt.AbstractModule):
  ...
  def _build(self, x):
    ...
    x = snt.Linear(256)(x) #X: (n, 5, length)

    #cluster_encoding = gaussian_encoder(x) #(M, self._n_caps, 256)
    cluster_encoding = snt.Linear(512)(x) #(M, 5, 512)
    cluster_encoding = tf.nn.dropout(tf.nn.softplus(cluster_encoding), 0.9)
    ...
    if self._encoder_type == 'linear':
      ...
      #h = snt.BatchFlatten()(cluster_encoding)
      h = snt.Linear(n_dims)(cluster_encoding)
      h = tf.nn.dropout(tf.nn.softplus(h), 0.9) #(N, self._n_caps, n_dims)

    else:
      h = snt.AddBias(bias_dims=[2, 3])(cluster_encoding)
      ...
      if self._encoder_type == 'conv':
        ...
        h = snt.Conv2D(64, 1, 1)(h) #(N, 5, 128, 64)
        h = tf.reshape(h, [batch_size, 5, 128*64])
        h = snt.BatchApply(snt.Linear(n_dims * self._n_caps + self._n_caps))(h)

        h, a = tf.split(h, [n_dims * self._n_caps, self._n_caps], -1) #(N, 640, 5888)

        a = tf.nn.softmax(a, 1) #(N, 5, 256)
        a = tf.reshape(a, [batch_size, 5, 1, self._n_caps])
        h = tf.reshape(h, [batch_size, 5, n_dims, self._n_caps])
        h = h * a
        h = tf.nn.softplus(h)
    ...
    #h = tf.reshape(h, [batch_size, 5, self._n_caps, n_dims])
    ...
    feature = snt.Linear(128)(feature)  # (N, self._n_caps, n_fea)

    pres = tf.nn.sigmoid(pres_logit)  # (N, self._n_caps, 1)
    pose = math_ops.geometric_transform(pose, self._similarity_transform)#(N, self._n_caps, 6)
    pose = tf.nn.softplus(pose)
    pose = snt.Linear(128)(pose)
    #pose = tf.nn.dropout(tf.nn.relu(pose), 0.9)

    return self.OutputTuple(pose, feature, pres, pres_logit, cluster_encoding, h)
...
class TemplateBasedMaterialDecoder(snt.AbstractModule):
  ...
  def _build(self,
    ...
    if self._output_pdf_type == 'mixture':
      ...
      #template_mixing_logits = tf.reshape(template_mixing_logits, [32, 17, -1, 1])
      #template_mixing_logits = tf.transpose(template_mixing_logits, [3, 0, 1, 2])
      #transformed_templates = tf.reshape(transformed_templates, [32, 17, -1, 1])
      #transformed_templates = tf.transpose(transformed_templates, [3, 0, 1, 2])

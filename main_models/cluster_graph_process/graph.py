...
from cluster_graph_process import local_env
from models.submodels import get_graphs_within_cutoff, expand_1st, to_list
...
class StructureGraph(MSONable):
    ...
    def get_flat_data(graphs: List[Dict], targets: List = None) -> tuple:
        ...
        for feature in ["comp_w", "elem_num", "atom_fea", "atom", "bond_0", "state", "index1_0", "index2_0",
                        "bond_1", "index1_1", "index2_1", "bond_2", "index1_2", "index2_2", "bond_3", "index1_3", "index2_3"]:
...
class StructureGraphFixedRadius(StructureGraph):
    ...
    def convert(self, structure: Structure, state_attributes: List = None) -> Dict:
        ...
        Args:
            ...
            (dictionary)
        """
        # element embedding block
        def create_cluster(atom_num, index1, index2, bonds, n_clusters):
            c_dis = []
            c_index = []
            c_nei_index = []
            for i in range(atom_num):
                dis = []
                nei_index = []
                for (j, k, m) in zip(index1, index2, bonds):
                    if j == i:
                        dis.append(m)
                        nei_index.append(k)

                sort_index = np.array(dis).argsort()
                for c in range(n_clusters):
                    nei_index = np.array(nei_index)
                    dis = np.array(dis)
                    index = sort_index[c]
                    c_index.append(i)
                    c_nei_index.append(nei_index[index])
                    c_dis.append(dis[index])

            return c_dis, c_index, c_nei_index
        ...
        for i in elements_dic:
            mass_dict.append(Formula(i).mass/100)

        mass_list = []
        n = 0
        for j in list(comp_dict.values()):
            mass_list = mass_list + [mass_dict[n]]*int(j)
            n += 1
        ...
        # Multiple body clusters extraction

        two_dis, two_index, two_nei_index = create_cluster(atom_num, index1,
                                                           index2, bonds, n_clusters=2)

        three_dis, three_index, three_nei_index = create_cluster(atom_num, index1,
                                                                 index2, bonds, n_clusters=3)

        fiv_dis, fiv_index, fiv_nei_index = create_cluster(atom_num, index1,
                                                           index2, bonds, n_clusters=5)

        sev_dis, sev_index, sev_nei_index = create_cluster(atom_num, index1,
                                                           index2, bonds, n_clusters=7)

        two_cluster_bonds = two_dis
        two_atom_index = two_index
        two_nei_index = two_nei_index

        three_cluster_bonds = three_dis
        three_atom_index = three_index
        three_nei_index = three_nei_index

        fiv_cluster_bonds = fiv_dis
        fiv_atom_index = fiv_index
        fiv_nei_index = fiv_nei_index

        sev_cluster_bonds = sev_dis
        sev_atom_index = sev_index
        sev_nei_index = sev_nei_index



        n = len(three_atom_index)


        if np.size(np.unique(index1)) < len(atoms):
            raise RuntimeError("Isolated atoms found in the structure")
        return {"comp_w": weights, "elem_num": elements_num, "atom_fea": atom_fea,
                "atom": atoms, "bond_0": two_cluster_bonds, "state": mass_list,
                "index1_0": two_atom_index, "index2_0": two_nei_index, 'atom_n': n,
                "bond_1": three_cluster_bonds, "index1_1": three_atom_index, "index2_1": three_nei_index,
                "bond_2": fiv_cluster_bonds, "index1_2": fiv_atom_index, "index2_2": fiv_nei_index, # cluster_2
                "bond_3": sev_cluster_bonds, "index1_3": sev_atom_index, "index2_3": sev_nei_index
                }# cluster_3
...
class BaseGraphBatchGenerator(Sequence):
    ...
    def _combine_graph_data(
        ...
        # clusters data
        connection_list_temp_1: List[np.ndarray],
        index1_temp_1: List[int],
        index2_temp_1: List[int],
        connection_list_temp_2: List[np.ndarray],
        index1_temp_2: List[int],
        index2_temp_2: List[int],
        connection_list_temp_3: List[np.ndarray],
        index1_temp_3: List[int],
        index2_temp_3: List[int],
        ...
        gbond_1 = []
        for i, j in enumerate(connection_list_temp_1):
            gbond_1 += [i] * len(j)

        gbond_2 = []
        for i, j in enumerate(connection_list_temp_2):
            gbond_2 += [i] * len(j)

        gbond_3 = []
        for i, j in enumerate(connection_list_temp_3):
            gbond_3 += [i] * len(j)
        ...
        connection_list_temp_1 = np.concatenate(connection_list_temp_1, axis=0)
        connection_list_temp_1 = self.process_bond_feature(connection_list_temp_1)

        connection_list_temp_2 = np.concatenate(connection_list_temp_2, axis=0)
        connection_list_temp_2 = self.process_bond_feature(connection_list_temp_2)

        connection_list_temp_3 = np.concatenate(connection_list_temp_3, axis=0)
        connection_list_temp_3 = self.process_bond_feature(connection_list_temp_3)
        ...
        global_list_temp = np.concatenate(global_list_temp, axis=0)
        global_list_temp = self.process_atom_feature(global_list_temp)
        ...
        index2_3 = []
        offset_ind = 0
        for ind1, ind2, clt, cai, nfl, cl, ind1_1, ind2_1, ind1_2, ind2_2, ind1_3, ind2_3 in zip(index1_temp, index2_temp, comp_list_temp,
                                                 crystal_atom_idx, self_fea_list, nbr_fea_list,
                                                 index1_temp_1, index2_temp_1, index1_temp_2, index2_temp_2,
                                                                                  index1_temp_3, index2_temp_3):
            ...
            index1_1 += [i + offset_ind for i in ind1_1]
            index2_1 += [i + offset_ind for i in ind2_1]

            index1_2 += [i + offset_ind for i in ind1_2]
            index2_2 += [i + offset_ind for i in ind2_2]

            index1_3 += [i + offset_ind for i in ind1_3]
            index2_3 += [i + offset_ind for i in ind2_3]
        ...
        inputs = (expand_1st(np.array(comp_list).reshape(-1)),
                  ...
                  expand_1st(np.array(gbond, dtype=np.int32)),
                  # cluster_1
                  expand_1st(connection_list_temp_1),
                  expand_1st(np.array(index1_1, dtype=np.int32)),
                  expand_1st(np.array(index2_1, dtype=np.int32)),
                  expand_1st(np.array(gbond_1, dtype=np.int32)),
                  # cluster_2
                  expand_1st(connection_list_temp_2),
                  expand_1st(np.array(index1_2, dtype=np.int32)),
                  expand_1st(np.array(index2_2, dtype=np.int32)),
                  expand_1st(np.array(gbond_2, dtype=np.int32)),
                  # cluster_3
                  expand_1st(connection_list_temp_3),
                  expand_1st(np.array(index1_3, dtype=np.int32)),
                  expand_1st(np.array(index2_3, dtype=np.int32)),
                  expand_1st(np.array(gbond_3, dtype=np.int32)),
...
class GraphBatchGenerator(BaseGraphBatchGenerator):
    ...
    def __init__(
        ...
        bond_features_1: List[np.ndarray],
        index1_list_1: List[int],
        index2_list_1: List[int],
        bond_features_2: List[np.ndarray],
        index1_list_2: List[int],
        index2_list_2: List[int],
        bond_features_3: List[np.ndarray],
        index1_list_3: List[int],
        index2_list_3: List[int],
        ...
        self.bond_features_1 = bond_features_1
        self.index1_list_1 = index1_list_1
        self.index2_list_1 = index2_list_1
        self.bond_features_2 = bond_features_2
        self.index1_list_2 = index1_list_2
        self.index2_list_2 = index2_list_2
        self.bond_features_3 = bond_features_3
        self.index1_list_3 = index1_list_3
        self.index2_list_3 = index2_list_3
    ...
    def _generate_inputs(self, batch_index: list) -> tuple:
        ...
        # cluster_1
        connection_list_temp_1 = itemgetter_list(self.bond_features_1, batch_index)
        index1_temp_1 = itemgetter_list(self.index1_list_1, batch_index)
        index2_temp_1 = itemgetter_list(self.index2_list_1, batch_index)
        # cluster_2
        connection_list_temp_2 = itemgetter_list(self.bond_features_2, batch_index)
        index1_temp_2 = itemgetter_list(self.index1_list_2, batch_index)
        index2_temp_2 = itemgetter_list(self.index2_list_2, batch_index)
        # cluster_3
        connection_list_temp_3 = itemgetter_list(self.bond_features_3, batch_index)
        index1_temp_3 = itemgetter_list(self.index1_list_3, batch_index)
        index2_temp_3 = itemgetter_list(self.index2_list_3, batch_index)

        targets = itemgetter_list(self.targets, batch_index)

        return comp_list_temp, ele_list_temp, atom_list_temp, \
               feature_list_temp, connection_list_temp, global_list_temp, \
               index1_temp, index2_temp, connection_list_temp_1, index1_temp_1, index2_temp_1,\
               connection_list_temp_2, index1_temp_2, index2_temp_2,\
               connection_list_temp_3, index1_temp_3, index2_temp_3, targets # cluster_2
...
class GraphBatchDistanceConvert(GraphBatchGenerator):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            bond_features_1=bond_features_1,
            index1_list_1 = index1_list_1,
            index2_list_1 = index2_list_1,

            bond_features_2=bond_features_2,
            index1_list_2 = index1_list_2,
            index2_list_2 = index2_list_2,

            bond_features_3=bond_features_3,
            index1_list_3 = index1_list_3,
            index2_list_3 = index2_list_3,
